...
class HostFootballTournament(APIView):
    ...
    def post(self,request,*args,**kwargs):
       try: 
            ...
            if HostFootballTournaments.objects.filter(host=request.user.id,date=store_date,district = store_district):
                ...
                if verify_profile == True:
                    ...
                    user            = HostFootballTournaments.objects.create(host=account)
                    ...
                    return Response({"Your profile is selected for other sports item but not as for Football "})
...
class ListTournamentView(APIView):
    ...
    authentication_classes  = (JWTAuthentication,)
    permission_classes      = (IsAuthenticated,)
    quertset                = HostFootballTournaments.objects.all()
    
    def get(self,request,pk):
        
        try:
            if pk == 0:
                tournament = HostFootballTournaments.objects.all()
                ...
                if HostFootballTournaments.objects.filter(id=pk):
                    tournament = HostFootballTournaments.objects.get(id=pk)
...
class TournamentUpdateDelete(APIView):
    ...
    def put(self,request,pk,*args,**kwargs) :
        ...
        try:
            ...
            if HostFootballTournaments.objects.filter(id=pk).filter(host=user):
                tournament      = HostFootballTournaments.objects.get(id=pk)
    ...
    def delete(self,request,pk):
        try:
            if HostFootballTournaments.objects.filter(id=pk):
                if HostFootballTournaments.objects.filter(id=pk,host=request.user.id):
                ...
                if Tournament_Notifications.objects.filter(tournament=pk,verified =True):
                    ...
                    if user_history.owner == True: 
                        ...
                        if Resgister_Tournaments.objects.filter(tournament=pk,registered=getClub.id):
                            ...
                            getTournament = HostFootballTournaments.objects.get(id=pk)
